 ...
 * Allows restaurant owners to authenticate via Google, Facebook OAuth or Email/Password
 */
...
export default function RestaurantLogin() {
  ...
  // Extract error and token from URL params
  ...
  const handleFacebookLogin = () => {
    const authUrl = `/api/auth/facebook${token ? `?token=${token}` : ""}`;
    window.location.href = authUrl;
  };
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setEmailError(data.error || "Identifiants invalides");
  ...
  const getErrorMessage = (errorCode: string | null) => {
    switch (errorCode) {
      ...
      case "facebook_auth_failed":
        return "La connexion avec Facebook a échoué. Veuillez réessayer.";
      case "invitation_expired":
        return "Votre lien d'invitation a expiré. Veuillez contacter l'administrateur.";
      case "invitation_invalid":
        return "Lien d'invitation invalide. Veuillez contacter l'administrateur.";
      default:
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            <Button
              ...
              variant="outline"
              className="w-full h-11 text-sm font-medium hover:bg-gray-50 transition-colors"
            >
