...
export default function BusinessPublicPage() {
  ...
  const business = trpc.businesses.getPublicBySlug.useQuery({ slug }, { retry: false });
  const catalog = trpc.businesses.getPublicCatalogBySlug.useQuery({ slug }, { retry: false });
  const itemsByCollection = useMemo(() => {
    const groups = new Map<number | null, typeof catalog.data extends infer T ? T extends { items: infer I } ? I : never : never>();
    for (const item of catalog.data?.items ?? []) groups.set(item.collectionId, [...(groups.get(item.collectionId) ?? []), item]);
    return groups;
  }, [catalog.data]);

  if (business.isLoading || catalog.isLoading) return <div className="min-h-[100dvh] bg-background" />;
  if (!business.data) return <div className="min-h-[100dvh] bg-background" />;

  const profile = business.data.profile;
  return (
    <main className="min-h-[100dvh] bg-[#fbf8f3] text-foreground">
      <BusinessPublicChrome
        name={profile?.displayName || business.data.slug}
      ...
      <section className="relative flex min-h-[58dvh] items-end overflow-hidden px-5 pb-14 pt-28 sm:px-8">
        ...
        <div className="relative mx-auto w-full max-w-5xl text-white">
          <p className="mb-4 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-white/70"><Sparkles className="h-4 w-4" /> {verticalLabel[business.data.vertical] || "Entreprise"}</p>
          <h1 className="max-w-3xl font-display text-5xl leading-[0.94] sm:text-7xl">{profile?.displayName || business.data.slug}</h1>
      ...
      <section id="catalogue" className="mx-auto max-w-5xl px-5 py-16 sm:px-8 sm:py-24">
        <div className="mb-10 flex flex-wrap items-end justify-between gap-5">
          <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">{catalog.data?.catalog?.type === "products" ? "Collections" : "Prestations"}</p><h2 className="mt-2 font-display text-4xl">{catalog.data?.catalog?.name || "Notre sélection"}</h2></div>
        ...
        {catalog.data?.collections.map((collection) => <div key={collection.id} className="mb-12 last:mb-0">
        ...
        {!catalog.data?.catalog && <div className="rounded-[1.45rem] border border-dashed p-8 text-muted-foreground">Ce catalogue sera disponible prochainement.</div>}
      </section>
