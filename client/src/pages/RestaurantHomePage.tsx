  ...
  return (
    <ThemeWrapper theme={storefrontTheme}>
      <PublicVitrineChrome name={restaurant.name} logoUrl={restaurant.logoUrl}>
        <div className="flex items-center gap-1.5">{(restaurant.subscriptionTier === "pro" || restaurant.subscriptionTier === "premium") && <LanguageSelector currentLanguage={currentLanguage} onLanguageChange={setCurrentLanguage} className="h-10 rounded-[1rem] border-transparent bg-black/[0.05] px-3 shadow-none hover:bg-black/[0.08]" />}<div className="hidden items-center gap-1.5 md:flex"><button type="button" onClick={() => navigate(`/${slug}/menu`)} className="h-10 rounded-[1rem] px-3 text-sm font-medium text-foreground transition-[background-color,transform] duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] hover:bg-black/[0.05] active:scale-[0.98]">Menu</button><button type="button" onClick={() => setIsReservationOpen(true)} className="h-10 rounded-[1rem] px-3 text-sm font-medium text-foreground transition-[background-color,transform] duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] hover:bg-black/[0.05] active:scale-[0.98]">Réserver</button></div></div>
      </PublicVitrineChrome>
