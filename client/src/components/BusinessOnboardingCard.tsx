...
export function BusinessOnboardingCard({ businessId, onNavigate }: { businessId: number | null; onNavigate: (tab: "menu" | "settings" | "import") => void }) {
  ...
  }, [onboarding?.industry, onboarding?.primaryGoal]);
  ...
  const saveSetup = () => update.mutate({ businessId, industry: industry as "restaurant" | "beauty" | "retail" | "service" | "events" | "other", primaryGoal: goal || "Présenter mon activité", completedSteps: mergedSteps("business_type"), status: "in_progress" });
