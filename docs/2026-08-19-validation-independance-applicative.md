...
Le formulaire public de demande d’appel ou de signalement possède un budget propre de **3 soumissions par 15 minutes et par adresse IP**. Ses champs sont normalisés et bornés : nom à 120 caractères, téléphone à 40 caractères, adresse électronique à 320 caractères et message à 2 000 caractères. Les régressions confirment le refus d’une quatrième soumission ainsi que le maintien de l’accès aux procédures tRPC adjacentes.

Les demandes d’assistance utilisent désormais le même exécuteur reCAPTCHA client que le formulaire de contact. Le jeton, lié à l’action `submit_assistance_request`, est vérifié côté serveur avant toute insertion en base ou notification ; un échec entraîne un refus `FORBIDDEN`. En développement, seul le jeton de bypass déjà contrôlé est accepté, ce qui permet la validation locale sans charger de badge invalide.

Le chatbot public propre à chaque établissement bénéficie aussi d’un budget de **10 messages par minute et par IP** avant de charger son contexte de catalogue et d’appeler le modèle. Son entrée reste bornée à 800 caractères. Les métadonnées de conversation ne sont plus fournies par le navigateur : l’adresse IP et l’agent utilisateur proviennent exclusivement de la requête vue par le serveur, avec un agent borné à 512 caractères.

Le formulaire public de contact conserve sa vérification reCAPTCHA et dispose maintenant d’un plafond complémentaire de **3 soumissions par 15 minutes et par IP** avant la notification propriétaire. Ses données sont normalisées et bornées : nom 2–120 caractères, téléphone 6–40 caractères, email 320 caractères et message optionnel 2 000 caractères. Les régressions confirment le refus de la quatrième soumission sans impact sur les procédures publiques adjacentes.

Le suivi des vues publiques n’accepte plus d’adresse IP, d’agent utilisateur ou de référent fournis par le navigateur. Le client ne transmet que l’établissement et le chemin visité ; le serveur dérive les métadonnées de sa propre requête et les borne avant l’écriture analytique. Le chemin est lui-même limité à 2 048 caractères.

Chaque adresse IP peut désormais enregistrer jusqu’à **20 vues publiques par minute**. Ce budget couvre largement une navigation normale tout en empêchant un script isolé de gonfler rapidement les indicateurs. La régression valide le refus de la vingt-et-unième vue tout en conservant l’accès aux autres procédures publiques.
